# タスクリスト: カテゴリ別インデックスページの作成

- [x] `task.md` と `implementation_plan.md` アーティファクトの作成
- [x] リクエスト通りにアーティファクトのコピーを `roadmap/` ディレクトリに保存
- [x] 再利用可能な `ArticleCard.astro` コンポーネントの作成
    - [x] タイトル、著者、説明文の表示
    - [x] OGP画像サムネイルの表示（フロントマターの `ogImage` を使用）
    - [x] レスポンシブデザインのスタイル適用
- [x] `src/pages/[lang]/blog/index.astro` の作成
    - [x] 全ての `blog` コレクションエントリの取得
    - [x] ArticleCardsのグリッド/リスト描画
    - [x] タイトルの左揃え対応
- [x] `src/pages/[lang]/newsrelease/index.astro` の作成
    - [x] 全ての `newsrelease` コレクションエントリの取得
    - [x] ArticleCardsのグリッド/リスト描画
    - [x] タイトルの左揃え対応
- [x] `src/pages/[lang]/notes/index.astro` の作成
    - [x] 全ての `notes` コレクションエントリの取得
    - [x] ArticleCardsのグリッド/リスト描画
    - [x] タイトルの左揃え対応
- [x] ブラウザでのページ確認
    - [x] `/jp/blog`, `/jp/newsrelease`, `/jp/notes` の確認
    - [x] レスポンシブレイアウトの確認
    - [x] タイトル左揃えの確認
